...
ds2019 = xr.open_dataset(ifile_cached)
...
if not os.path.exists(ifile_cached): run_shell(f'cp {ifile} {ifile_cached}')
ds2018 = xr.open_dataset(ifile_cached)
print('[loaded]:', ifile_cached, '**from**', ifile) 
...
if __name__ == '__main__':
    ...
    ds2019['r'].plot.hist(bins=bins, density=True, alpha=alpha, ax=ax, label=f'LMR2.1')
    ds2018['r'].plot.hist(bins=bins, density=True, alpha=alpha, ax=ax, label=f'LMR2.0')

    ax.set_ylabel('PDF')
    ax.legend(loc='upper left')
    ax.set_title('Correlation between sediment HUs and LMR SST emulated HUs')
