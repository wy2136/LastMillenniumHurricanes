...
def wyplot(yearc, rSST=False, ax=None, **kws):
    ...
    ifile = f'./data/maps_ssta/map_ssta_LME_{years.start}-{years.stop}_ref850-1850.nc'
    if rSST:
    ...
    da.plot.contourf(ax=ax, levels=levels, cmap='RdBu_r', extend='both', vmax=vmax, **kws)
    ax.set_title(f'LME {years.start}-{years.stop} minus 850-1850')
...
if __name__ == '__main__':
    ...
    ax = axes[0,0]
    wyplot(1277, ax=ax, transform=dproj)
    ax.add_feature(cfeature.LAND, color=land_color)

    ax = axes[0,1]
    wyplot(1277, rSST=True, ax=ax, transform=dproj)
    ax.add_feature(cfeature.LAND, color=land_color)

    ax = axes[1,0]
    wyplot(1980, ax=ax, transform=dproj)
    ax.add_feature(cfeature.LAND, color=land_color)

    ax = axes[1,1]
    wyplot(1980, rSST=True, ax=ax, transform=dproj)
    ax.add_feature(cfeature.LAND, color=land_color)
