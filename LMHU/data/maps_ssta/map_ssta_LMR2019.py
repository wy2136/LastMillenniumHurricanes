...
da = xr.open_dataarray(ifile).load()
da = da.mean('en')
dc['LME_fullForcing'] = da
#LME_GHG
ifile = 'data/b.e11.BLMTRC5CN.f19_g16.GHG.001-003.cam.h0.TS.0850-2005.nc'
print(ifile, 'loading...')
da = xr.open_dataarray(ifile).load()
da = da.mean('en')
dc['LME_GHG'] = da
...
def wyplot(ax=None, **kws):
    if ax is None:
        fig, ax = plt.subplots()
    da.plot.contourf(levels=17, cmap='RdBu_r', extend='neither', **kws)
    plt.title(f'LMR2.1 {years.start}-{years.stop} minus {yearsRef.start}-{yearsRef.stop}')
...
if __name__ == '__main__':
    ...
    wyplot(ax=ax, transform=dproj, vmax=0.32)
