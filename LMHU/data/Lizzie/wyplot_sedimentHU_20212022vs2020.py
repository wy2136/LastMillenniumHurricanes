...
da2021 = xr.open_dataarray('sedimentHU_v20211203_smooth40yr.nc')
da_wCaySal = xr.open_dataarray('sedimentHU_v20220103_wCaySal_smooth40yr.nc')
da_noCaySal = xr.open_dataarray('sedimentHU_v20220103_noCaySal_smooth40yr.nc')
...
def do_corrcoef(yearspan):
    ...
    da1 = da2021.sel(member='sediment_estimate').sel(year=yearspan)
    da2 = da_wCaySal.sel(member='sediment_estimate').sel(year=yearspan)
    da3 = da_noCaySal.sel(member='sediment_estimate').sel(year=yearspan)
...
if __name__ == '__main__':
    ...
    da = da2021.sel(member=['lower_estimate', 'upper_estimate']).pipe(sel_years)
    ax.fill_between(da.year, *da.transpose(), color='C1', alpha=0.2)
    da = da2021.sel(member='sediment_estimate').pipe(sel_years)
    da.plot(ax=ax, label='v2021')
