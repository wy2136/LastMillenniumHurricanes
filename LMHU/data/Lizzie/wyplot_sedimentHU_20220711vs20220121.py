...
da_noCaySal_20210121 = xr.open_dataarray('sedimentHU_v20220121_noCaySal_smooth40yr.nc')
da_noCaySal = xr.open_dataarray('sedimentHU_v20220711_noCaySal_smooth40yr.nc')
da_wCaySal = xr.open_dataarray('sedimentHU_v20220711_wCaySal_smooth40yr.nc')
...
def do_corrcoef(yearspan):
    ...
    da0 = da_noCaySal_20210121.sel(member='sediment_estimate').sel(year=yearspan)
...
if __name__ == '__main__':
    ...
    da = da_noCaySal_20210121.sel(member=['lower_estimate', 'upper_estimate']).pipe(sel_years)
    ax.fill_between(da.year, *da.transpose(), color='C0', alpha=0.2)
    da = da_noCaySal_20210121.sel(member='sediment_estimate').pipe(sel_years)
    da.plot(ax=ax, label='noCaySal_v20210121')
    ...
    da.plot(ax=ax, label='noCaySal(12)')
    ...
    da.plot(ax=ax, label='wCaySal(13)', ls='--')

    ax.legend(loc='upper left', ncol=3)
    #cc850_2000 = do_corrcoef(slice(850, 2000)) #correlation coefficient
    r1, r2 = do_corrcoef(yearsPI) #correlation coefficient
    ax.set_title(f'corr coef with v20210121: {r1:.2g}(noCaySal);{r2:.2g}(wCaySal)')
