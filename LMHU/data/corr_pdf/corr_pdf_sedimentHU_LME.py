...
if __name__ == '__main__':
    ...
    reg.r.sel(ens=slice(1,None)).plot.hist(bins=np.arange(-0.6,0.61,0.01), alpha=alpha, ax=ax, label=f'LME {lme_case} {n_ens}ens members')
    reg.r.sel(ens=0).plot.hist(bins=np.arange(-0.6,0.61,0.01), alpha=alpha, ax=ax, label=f'LME {lme_case} ens mean')
    ax.set_ylabel('#')
    ax.legend()
    #savefig
    ...
    reg.r.sel(ens=slice(1,None)).plot.hist(bins=np.arange(-0.6,0.61,0.01), alpha=alpha, density=True, label=f'LME {lme_case} {n_ens}ens members')
    reg.r.sel(ens=0).plot.hist(bins=np.arange(-0.6,0.61,0.01), alpha=alpha, density=True, label=f'LME {lme_case} ens mean')
    ax.set_ylabel('PDF')
    ax.legend()
